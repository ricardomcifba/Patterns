...
public class MainPanel extends javax.swing.JPanel implements MouseListener, KeyListener {
    ...
    public void bodiesUpdated(ArrayList<Point2D> bodies) {
        ...
        for (Point2D point2d : bodies) {
            ...
            if (bodies.size() - 1 == i)//Substituindo o getUserData();
                // Player
                rectangle.setLocation((size.width/2-28 + (int) point2d.getX()), (size.width/2-28 + (int) point2d.getY()));         
            else
                // Block
                rectangle.setLocation((size.width/2-14 + (int) point2d.getX()), (size.width/2-14 + (int) point2d.getY()));
    ...
    public void paintComponent(Graphics g) {
        ...
        Collection<Rectangle> rec = new ArrayList();
        rec = m_p2dRect.values();
